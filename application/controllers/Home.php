...
class Home extends CI_Controller {
	...
	{	
		...
		$data['query'] = $this->Dashboard_model->isreview($userid);
		$data['isskip'] = $this->Dashboard_model->isskiprequired($userid);
		
		if($data['query'][0]['count']>0)
		{
	...
	{	
		...
		else if($this->input->post('type')=='ISLOGIN')
		{
			...
			{ 
				...
				{ */
					$this->session->set_userdata('schoolname',$data['userdetail'][0]['sname']);
				/* } */
				...
				{	
					$data['review'] = $this->Dashboard_model->isreview($this->session->userId);
					if($data['review'][0]['count']==0)
					{
					echo "REVIEW";exit;
					}
					echo "PROFILE";exit;
	...
	public function mygames()  
	{
		...
		redirect('index.php/home/review');
		}
		else 
		{
	...
	public function registration()  
	{
		...
		$data['CountryList'] = $this->Dashboard_model->CountryList();
		$this->load->view('header');
		...
		if($regtblid!='')
{
	...
	public function insertultrev()  
	{
		if($this->input->post('rdQ1')!='' && $this->input->post('rdQ2')!='' && $this->input->post('rdQ3')!='' && $this->input->post('rdQ4')!='' && $this->input->post('rdQ5')!='' && $this->input->post('rdQ6')!='' && $this->input->post('rdQ7[]')!='' && $this->input->post('rdQ8')!='' && $this->input->post('txtplan')!='' && $this->input->post('txtfeedback')!='')
		{ 
			
			  	$qryinsertRegister=$this->Dashboard_model->InsertReview($this->session->userId,$this->input->post('rdQ1'),$this->input->post('rdQ2'),$this->input->post('rdQ3'),$this->input->post('rdQ4'),$this->input->post('rdQ5'),$this->input->post('rdQ6'),implode('-',$this->input->post('rdQ7[]')),$this->input->post('rdQ8'),$this->input->post('txtplan'),$this->input->post('txtfeedback'));
	...
	public function skipultrev()  
	{
		 
			  	$qryinsertRegister=$this->Dashboard_model->InsertReview($this->session->userId,'','','','','','','','','','');
		...
		if($this->input->post('txtEmail')!='' && $this->input->post('txtOPassword')!='' && $this->input->post('ddlGrade')!='' && $this->input->post('txtCouponcode')!='')
		{
			...
			{ 
				...
				if($isallow==1)
				{
					...
					if($qryinsertRegister[0]['resp']>0)
					{
						$qryUserDetails=$this->Dashboard_model->InsertReview($qryinsertRegister[0]['resp'],'','','','','','','','','','');
